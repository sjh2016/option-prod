...
@FeignClient(value = "core-server", contextId = "BindCardAPI", qualifier = "bindCardAPI")
public interface BindCardAPI extends BaseAPI {

	@RequestMapping(value = "/bindcard/query", method = RequestMethod.GET)
	public Response<BindCardDTO> _query(@RequestParam("userId") Long userId, @RequestParam("id") Long id);
	...
	public Response<Void> _delete(@RequestParam("userId") Long userId, @RequestParam("id") Long id);

	public default BindCardDTO query(Long userId, Long id) {
		return getResponseData(_query(userId, id));
	}
	...
	public default void delete(Long userId, Long id) {
		getResponseData(_delete(userId, id));
	}
