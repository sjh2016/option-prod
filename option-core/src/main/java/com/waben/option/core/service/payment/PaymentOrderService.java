...
public class PaymentOrderService {
    ...
    void paymentComplete(PaymentOrder order) {
        ...
        log.info("1更新[{}]", JSON.toJSONString(transactionBeanList));
...
//                BigDecimal first = profitDivideRatio(1);
//                transactionBeanList.clear();
//                transactionBeanList
//                        .add(AccountTransactionBean.builder().userId(firstuser.getId()).type(TransactionEnum.CREDIT_SUBORDINATE)
//                                .amount(profitDivide(amount, first)).transactionId(order.getId()).currency(order.getTargetCurrency()).build());
//                accountService.transaction(firstuser.getId(), transactionBeanList, 1);
//                log.info("2更新[{}]", JSON.toJSONString(transactionBeanList));
//                modifyUser(firstuser.getId(), order.getRealNum());
//            }
//
//            //二级父
...
//                if (null != seconduser) {
//                    BigDecimal second = profitDivideRatio(2);
//                    transactionBeanList.clear();
//                    transactionBeanList
//                            .add(AccountTransactionBean.builder().userId(seconduser.getId()).type(TransactionEnum.CREDIT_SUBORDINATE)
//                                    .amount(profitDivide(amount, second)).transactionId(order.getId()).currency(order.getTargetCurrency()).build());
//                    log.info("3更新[{}]", JSON.toJSONString(transactionBeanList));
//                    accountService.transaction(seconduser.getId(), transactionBeanList, 2);
//                    modifyUser(seconduser.getId(), order.getRealNum());
//                }
//            }
//
//            //二级父
...
//                if (null != thirdUser) {
//                    BigDecimal third = profitDivideRatio(3);
//                    transactionBeanList.clear();
//                    transactionBeanList
//                            .add(AccountTransactionBean.builder().userId(thirdUser.getId()).type(TransactionEnum.CREDIT_SUBORDINATE)
//                                    .amount(profitDivide(amount, third)).transactionId(order.getId()).currency(order.getTargetCurrency()).build());
//                    log.info("4更新[{}]", JSON.toJSONString(transactionBeanList));
//                    accountService.transaction(thirdUser.getId(), transactionBeanList, 3);
//                    modifyUser(thirdUser.getId(), order.getRealNum());
//                }
//            }
//        } catch (Exception e) {
//            log.info("保存父级收益信息异常");
//        }
    ...
    private BigDecimal profitDivideRatio(int level) {
        ...
        query.eq(Config.KEY, "levelPayment");
    ...
    public PaymentOrder queryUserInvitePeopleAmount(Long userId) {
        LambdaQueryWrapper<PaymentOrder> qw = Wrappers.lambdaQuery();
        qw.select(PaymentOrder::getReqNumSum,
                PaymentOrder::getReqMoneySum,
                PaymentOrder::getRealNumSum,
                PaymentOrder::getRealMoneySum);
        qw.eq(PaymentOrder::getUserId, userId);
        qw.eq(PaymentOrder::getStatus, PaymentOrderStatusEnum.SUCCESS);
        List<PaymentOrder> paymentOrderList = paymentOrderDao.selectList(qw);
