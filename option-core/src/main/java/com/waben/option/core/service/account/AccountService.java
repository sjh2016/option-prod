...
public class AccountService {
    ...
    //    @ShardingTransactionType(value = TransactionType.XA)
    @Transactional(rollbackFor = Exception.class)
    public List<Long> statement(Long userId, List<AccountTransactionBean> transactionBeanList) {
    ...
    public synchronized List<Long> transaction(Long userId, List<AccountTransactionBean> transactionBeanList,
        ...
        synchronized (userId) {
            ...
            for (AccountTransactionBean transactionBean : transactionBeanList) {
                ...
                switch (transactionBean.getType().getCreditDebitType()) {
                    case CREDIT:
                        amount = credit(account, transactionBean, level);
                        break;
                    case DEBIT:
                        amount = debit(account, transactionBean);
                        break;
...
//				redisTemplate.opsForValue().set(key, account);
    ...
    @Transactional(rollbackFor = Exception.class)
    BigDecimal credit(Account account, AccountTransactionBean transactionBean, int level) {
        account.credit(transactionBean.getAmount());
        if (1 == level) {
...
//        return summaryDTOList;
    ...
    public BigDecimal queryLevel(Long userId, int level) {
        return accountDao.queryLevel(userId, level);
    }

    public BigDecimal queryLevelCount(Long userId) {
        return accountDao.queryLevelCount(userId);
    }
